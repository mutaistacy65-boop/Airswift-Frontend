...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!templateId || !recipientEmail) {
    return res.status(400).json({ message: 'templateId and recipientEmail are required' })
  }
  ...
  try {
    ...
    // For now, simulate email sending (in production, integrate with SendGrid, SES, etc.)
